...
inline void cpy_line_direct(const void *f, void *t)
{
    __m128i dummy = {0, 0};
    __asm volatile(
        "movdqa (%2), %0\n\t"
...
int pjt_target_cachebypass_memmove(void *dest, const void *source, int sz)
{
    ...
    if (sz > 128)
    {
        ...
        // printf("t=%p f=%p\n", t, f);
        int shift = 0;
        while (t+256 < endt - shift)
        // while (t+256 < endt )
        {
            // memmove(t, f, 64);
            // _mm_prefetch(f + 512, _MM_HINT_T0);
            // _mm_prefetch(f + shift, _MM_HINT_T1);
            // _mm_prefetch(f + shift + 64, _MM_HINT_T1);
            // _mm_prefetch(f + shift + 128, _MM_HINT_T1);
            // _mm_prefetch(f + shift + 192, _MM_HINT_T1);
            // cpy_nt_4_line_256_target_cachebypass(f, t);
            // t += 256;
            // f += 256;
            cpy_nt_4_line_256_target_cachebypass(f, t);
            // _movdir64b((void *)t, (const void *)f);
            // _movdir64b(t + 64, f + 64);
            // _movdir64b(t + 128, f + 128);
            // _movdir64b(t + 192, f + 192);
            t += 256;
            f += 256;
        }
        // printf("t=%p f=%p\n", t, f);
...
int pjt_source_cachebypass_memmove(void *dest, const void *source, int sz)
{
    ...
    if (sz > 128)
    {
        ...
        {
            // memmove(t, f, 64);
            _mm_prefetch(f + 1024, _MM_HINT_NTA);
