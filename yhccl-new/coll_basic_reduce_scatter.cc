...
int
mca_coll_yhccl_reduce_scatter_test(const void *sbuf, void *rbuf, const int *rcounts,
                                    struct ompi_datatype_t *dtype,
                                    struct ompi_op_t *op,
                                    struct ompi_communicator_t *comm,
                                    mca_coll_base_module_t *module)
{
    std::cout << "std::cout  mca_coll_yhccl_reduce_scatter **** test" << std::endl;
    return OMPI_SUCCESS;
}
 ...
 */
int
mca_coll_yhccl_reduce_scatter_intra(const void *sbuf, void *rbuf, const int *rcounts,
                                    struct ompi_datatype_t *dtype,
                                    struct ompi_op_t *op,
                                    struct ompi_communicator_t *comm,
                                    mca_coll_base_module_t *module)
{
    ...
    if ((op->o_flags & OMPI_OP_FLAGS_COMMUTE) &&
        ...
        /* temporary receive buffer.  See coll_yhccl_reduce.c for details on sizing */
        ...
        if (0 == rank) {
            /* temporary receive buffer.  See coll_yhccl_reduce.c for
               details on sizing */
            recv_buf_free = (char*) malloc(buf_size);
            recv_buf = recv_buf_free - gap;
            if (NULL == recv_buf_free) {
 ...
 */
int
mca_coll_yhccl_reduce_scatter_inter(const void *sbuf, void *rbuf, const int *rcounts,
                                    struct ompi_datatype_t *dtype,
                                    struct ompi_op_t *op,
                                    struct ompi_communicator_t *comm,
                                    mca_coll_base_module_t *module)
{
    ...
    /*
     * The following code yhcclally does an interreduce followed by a
