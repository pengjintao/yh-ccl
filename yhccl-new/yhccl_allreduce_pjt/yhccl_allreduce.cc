...
//  (void *datasend, void *datarecv, int count, int elem_sz, yhccl_op fp)
{
    // puts("该方法已经作废");
    yhccl_contexts *ctx = yhccl_contexts::_ctx;
...
void pipelined_dpml_cache_efficient(int count, int elem_sz, yhccl_op op, int *counts, int *starts, MPI_Datatype mpitype, MPI_Op mpi_op)
{
    ...
    for (int ss = 0; ss < count; ss += slice_ct)
    {
        if (sliceid % ctx->intra_node_procn == ctx->intra_node_rank)
        {
            ...
            if (ctx->inter_node_procn > 1 && countl > 0)
            {
                volatile float *p = ctx->larger_msg_allreduce_result_start_0 + ss * elem_sz;
        ...
        if (s >= 0 && s < total_steps)
        {
            ...
            if (childn > 0)
            {
                ...
                for (int j = 0; j < childn; j++)
                {
                    ...
                    float *a = ctx->neigbbor_buffers[child] + (s & 0x1) * step * elem_sz;
                    float * b = datasend + s * step * elem_sz;
                    c = ctx->neigbbor_buffers[ctx->intra_node_rank] + (s& 0x1)*step * elem_sz;
                    if(j == 0)
                    {
                        for (int h = 0; h < ct; h++)
                            c[h] = a[h] + b[h];
                ...
                // printf("rank=%d s=%d c=%lf ct=%d\n", ctx->global_rank, s,c[0],ct);
                // fflush(stdout);
            }
            else
            {
