...
int main(int argc, char **argv)
{
    ...
    char *buffer1;
    posix_memalign((void **)&buffer1, 4096, (1UL << 31));
    MPI_Barrier(MPI_COMM_WORLD);
    ...
    for (int intra_slice = 18; intra_slice <= 18; intra_slice += 1)
    {
        ...
        for (int sz = 16; sz <= 25; sz++)
        {
            ...
            if (allreduce_rank == 0)
                ...
                //通信模式
                //====================================================================
            for (int pjt = 3; pjt <= 3;pjt++)
            {
                ccl_ctx._ctxp->_allgather_opt.intra_slice_size = 1 << intra_slice;
                if (pjt >= 1)
                    ccl_ctx._ctxp->_allgather_opt.using_non_temporal_memory_access = 1;
                else
                    ccl_ctx._ctxp->_allgather_opt.using_non_temporal_memory_access = 0;

                if(pjt == 2)
                    ccl_ctx._ctxp->_allgather_opt.using_numa_feature = 1;
                else
                    ccl_ctx._ctxp->_allgather_opt.using_numa_feature = 0;
                int loopN = 2200;
                if (sz >= 16)
                    loopN = 800;
                if (sz >= 20)
