...
#endif
    for (int intra_slice = 20; intra_slice <= 20; intra_slice += 2)
    {
        if (allreduce_rank == 0)
            fprintf(stderr, "================================ intra_slice = %d ================================\n", intra_slice);
        for (int sz = 20; sz <= 30; sz++)
        {
            int size = 1 << sz;
            if (allreduce_rank == 0)
            ...
            for (int pjt = 1; pjt <= 2;pjt++)
            {
                ...
                if (pjt >= 1)
                    ccl_ctx._ctxp->_bcast_opt.using_non_temporal_memory_access = 1;
                else
                    ccl_ctx._ctxp->_bcast_opt.using_non_temporal_memory_access = 0;

                if(pjt == 2)
                    ccl_ctx._ctxp->_bcast_opt.using_numa_feature = 1;
                else
                    ccl_ctx._ctxp->_bcast_opt.using_numa_feature = 0;
                int loopN = 2200;
                if (sz >= 20)
                    loopN = 1600;
                if (sz >= 25)
