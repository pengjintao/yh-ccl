...
#endif
	int pjtn=8;
	for (int intra_slice = 17; intra_slice <= 17; intra_slice += 1)
		// for (int intra_slice = 18; intra_slice <= 18; intra_slice++)
		// l表示节点间片的比例大小
		for (int l = 0 ;l <= 0; l += 1)
			// pjt表示测试模式
			for (int pjt = 4; pjt <= 5; pjt += 1)
			// for (int pjt = 4; pjt >= 3; pjt--)
			{
				if (ccl_ctx._ctxp->global_rank == 0)
					fprintf(stderr, "---------------ReduceScatter-------pjt=%d--l=%d---procn=%d---intra_slice=%d-------------------------------\n", pjt, l, allreduce_procn, intra_slice);
				fflush(stdout);

				// sz表示数据数量
				for (int sz = 10; sz <= 26; sz += 1)
				{
					...
					{
						ccl_ctx._ctxp->_opt.mulit_leader_algorithm = R_ALL_REDUCE;
					}
					...
					{
						{
							for (int loop = 2; loop <12; loop++)
							{
								...
								int step = count / allreduce_procn;
                                int start = allreduce_rank * step;
                                for (int i = 0; i < step; i++)
								{
									if (corrention_check)
                                        if (abs(recvbuf[i] - (loop + (i + start) % and_v) * allreduce_procn) > 0.0001)
